...
def test_ensure_manual_drive_creates_and_repoints(tmp_path: Path) -> None:
    # Mirror umu's real layout: the compat-data root IS the wine prefix,
    # with registry files at the root and "pfx" a symlink to "." (see
    # umu_run.setup_pfx). <root>/dosdevices is what Wine enumerates.
    prefix = tmp_path / "prefix"
    ...
    # An old build's d: mapping plus Wine's own device link for the
    # SD card that hijacks the letter on some machines (/dev/sda).
    (prefix / "dosdevices").mkdir()
    (prefix / "dosdevices" / "d:").symlink_to(games_a)
    (prefix / "dosdevices" / "d::").symlink_to("/dev/null")

    assert ensure_manual_drive(prefix, games_a)
    link = prefix / "dosdevices" / "u:"
    assert link.is_symlink() and link.resolve() == games_a.resolve()
    # Via the pfx self-symlink both spellings are the same directory.
    assert (prefix / "pfx" / "dosdevices" / "u:").is_symlink()
    # The legacy d: mapping is dropped; Wine's device link is untouched.
    assert not (prefix / "dosdevices" / "d:").is_symlink()
    assert (prefix / "dosdevices" / "d::").is_symlink()
