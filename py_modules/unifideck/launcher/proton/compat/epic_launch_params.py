...
from unifideck.launcher.proton.infrastructure.container_escape import spawn_escaped
...
async def maybe_run_pre_launch(
    ...
    try:
        ...
        # Escaped like every other spawn: legendary would have run this on
        # the host, so inside Steam's pressure-vessel it has to hop out too
        # or it runs somewhere the user never meant it to.
        proc = await spawn_escaped(shlex.split(command), env=env)
