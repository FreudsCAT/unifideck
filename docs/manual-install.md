...
FRONTEND:
 ...
 15. the modal LISTS candidate .exes (scan of the D: folder AND the
...
in the D: folder, the prefix (C:), the installer's folder, and generally
...
| `manual_exe_candidates(game_id)` | Scans the D: folder and the prefix's `drive_c`, returns candidate `.exe`s for the modal |
...
1. **The library tile appears after restarting Steam** — Steam only reads
   ...
   shortcut).
2. **"Later" on the exe modal** → safe: any later run re-offers the
   modal, and the Downloads row has a "Select exe" button to finish the
