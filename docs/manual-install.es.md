...
FRONTEND:
 ...
 15. el modal LISTA los .exe candidatos (escaneo de la carpeta D: y del
...
en la carpeta D:, el prefijo (C:), la carpeta del instalador, y en general
...
| `manual_exe_candidates(game_id)` | Escanea la carpeta D: y el `drive_c` del prefijo y devuelve los `.exe` candidatos para el modal |
...
2. **"Later" en el modal del exe** → sin riesgo: cualquier ejecución
   posterior re-ofrece el modal, y la fila de Downloads tiene el botón
   «Seleccionar exe» para completarlo sin ejecutar nada.
