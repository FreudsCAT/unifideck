...
**A1. Wire `resync_artwork` end-to-end** *(closes G1)*
...
**A2. Add `fetch_artwork=False` skip path** *(closes G3)*
...
**A3. Cache snapshot/restore on cancel** *(closes G2)*
...
**B1. Add `SyncRequest` queue model** *(closes G4)*
...
**C1. Wire `BackgroundCompatFetcher`** *(closes G5)*
...
**C2. Add `SizeFetcher` service** *(closes G6)*
...
**C3. Sync activity events** *(closes G7)*
...
**D1. Add negative-cache sentinels** *(closes G8)*
...
**D2. VDF post-write dedup with metadata scoring** *(closes G9)*
...
**D3. Pre-force-sync cache backup** *(closes G15)*
...
**E1. Cancel checkpoints inside post-sync phases** *(closes G10)*
...
**E2. Legacy AppID migration** *(closes G13)*
...
**E3. Protected shortcut registry** *(closes G14)*
...
**E4. Sync log banners** *(closes G16)*
...
**F1. Configurable cooldown** *(addresses G11)*
...
**F2. Per-store refresh button** *(closes G17)*
...
- The frontend's `progress.metacritic_total` is actually whatever the *generic metadata* phase counter is — not Metacritic at all. Anywhere the locale says "Fetching Metacritic metadata…", the user is reading a lie.
...
**G.1 Drop the `metacritic_*` rename, restore separate UnifiDB and Metacritic counters** *(closes G20, G21, G22, G27)*
...
**G.2 Add the missing status transitions** *(closes G26)*
...
**G.3 Add missing i18n keys** *(closes G25)*
...
**H.1 Create `steamgriddb/match.py`** *(closes G23)*
...
**H.2 Create `steamgriddb/search.py`** *(closes G23, G29)*
...
**H.3 Create `steamgriddb/assets.py`** *(closes G24)*
...
**H.4 Create `steamgriddb/ranking.py`** *(closes G30, G31)*
...
**H.5 Create `steamgriddb/batch.py`** *(closes G28)*
...
**H.6 Refactor `steamgriddb/client.py`** *(thin orchestrator)*
...
**H.7 Update `services/artwork/`** *(integration)*
...
**H.8 Wire to current event/cache/log conventions** *(architecture compliance)*
...
### 9.1 The chain that *should* fire
...
The `for done, game in enumerate(games, start=1):` loop catches exceptions per-game. The `await self._bus.emit(POST_SYNC_PHASE_CHANGED, ...)` at line 142-145 sits *after* the loop, at function level. If an exception escapes the per-game handlers (e.g. the bus itself raises, or `asyncio.CancelledError` propagates), the emit never runs. No try/finally guards it.
...
When the artwork phase ends, this calls `start_metadata(total=artwork_count)`. But metadata runs over *all* games, not just games that needed artwork. So `metadata_total` is set too low. When MetadataService then ticks `metadata_synced` past `metadata_total`, progress overshoots — and worse, `_recalc()` at [sync_progress.py:168-170](py_modules/unifideck/core/sync_progress.py#L168-L170) computes `sub = synced/total` which can exceed 1.0, producing a percentage > 98%.
...
For *that* user experience to look like "sync doesn't close immediately", the bar may also be **stuck at a high % because the lock didn't release** — that happens when:
...
**G32.1** Wrap the entire MetadataService `_run_enrichment` body in a try/finally that *always* emits `POST_SYNC_PHASE_CHANGED(phase="metadata", active=False)` on exit (success, exception, cancellation):
...
**G32.3** Fix the cross-wired `start_metadata(artwork_total)` in `_on_post_sync_phase`. The metadata service should call `progress.start_metadata(len(games))` from inside `_run_enrichment` *before* the loop — owning its own total. Remove [sync_service.py:478-479](py_modules/unifideck/core/sync_service.py#L478-L479) entirely.
